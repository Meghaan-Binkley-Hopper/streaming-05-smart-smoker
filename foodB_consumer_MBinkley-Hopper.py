...
def callback(ch, method, properties, body):
    ...
    foodB_message = body.decode()
    # decode the binary message body to a string
    print(f" [x] Received {foodB_message}")
    ...
    foodB_deque.append(foodB_message)

    """Set first temperature from deque for calculation below"""
    #declare position
    first_temp = foodB_deque[0] 
    ...
    if len(foodB_deque) == 20:
        ...
        if temp_assessment <= temp_alert:
            print("Alert! Food B's temperature has stalled.")
        else:
            print("Food B's temperature is as expected")
