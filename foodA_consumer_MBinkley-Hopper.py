...
"""Define Variables"""
#Food alerts when temperature has change of 1 or less degrees in 10 min
temp_alert = 1
hn = "localhost"
# limited to the the 20 most recent readings (10 minutes * 1 reading / 0.5 minutes)
...
def callback(ch, method, properties, body):
    ...
    foodA_message = body.decode()
    # decode the binary message body to a string
    print(f" [x] Received {foodA_message}")
    ...
    foodA_deque.append(foodA_message)

    """Set first temperature from deque for calculation below"""
    #declare position
    first_temp = foodA_deque[0] 
    ...
    if len(foodA_deque) == 20:
        #subtract first temp from current temp and round to see if temp has stalled
        ...
        if temp_assessment <= temp_alert:
            print("Alert! Food A's Temperature has stalled.")
        else:
            print("Food A's temperature is as expected")
